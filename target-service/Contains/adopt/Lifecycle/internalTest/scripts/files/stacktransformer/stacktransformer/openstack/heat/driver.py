...
class HeatDriver():
    ...
    def abandon_stack(self, stack_id):
        heat_client = self.__get_heat_client()
        logger.debug('Abandoning stack %s')
        abandon_result = heat_client.stacks.abandon(stack_id)
        return abandon_result

    def adopt_stack(self, stack_name, adoption_data, input_properties={}):
        if stack_name is None:
            raise ValueError('stack_name must be provided')
        if adoption_data is None:
            raise ValueError('adoption_data must be provided')
        heat_client = self.__get_heat_client()
        logger.debug('Adopting stack, creating with name %s', stack_name)
        create_result = heat_client.stacks.create(stack_name=stack_name, adopt_stack_data=adoption_data, parameters=input_properties)
        stack_id = create_result['stack']['id']
        logger.debug('Stack with name %s created and assigned id %s', stack_name, stack_id)
        return stack_id